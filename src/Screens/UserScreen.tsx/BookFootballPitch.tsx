...
const BookFootballPitch: React.FC<Props> = ({navigation,route}) => {
  ...
  //  console.log( tomorrow.getDate())
  let arr: number[] = [];
  //  console.log("sss=="+ new Date().getDate())
...
//           style={[Styles.dayofMonth, { backgroundColor }]}
//         >
//           <Text style={{ color }}>{item.day}</Text>
//         </TouchableOpacity>
//       </View>
//     );
//   };
        ...
        <View style={Styles.header_typePitch}>
          <View style={Styles._typePitch}>
            <Picker
              ...
              onValueChange={(itemValue, Index) => setSelectedPitch(itemValue)}
              style={{ height: 40 }}
            >
              <Picker.Item label="Sân 5 người" value="Sân 5 người" />
              <Picker.Item label="Sân 7 người" value="Sân 7 người" />
            </Picker>
          </View>
        </View>
        <View style={Styles.calendar}>
          <View style={Styles.calendar_block}>
            {/* <Icon  name ="chevron-left" size={22} style={Styles.btn_navigation} />
                        <Text style={[Styles.dayofMonth,{backgroundColor:'#1E90FF',color:'orange'}]}>{currentDate}</Text>
                        <Text style={Styles.dayofMonth}>{tomorrow.getDate()}</Text>
                        <Text style={Styles.dayofMonth}>{tomorrow1.getDate()}</Text> */}
            {/* <Text style={Styles.dayofMonth}>{tomorrow2.getDate()}</Text> */}
            {/* <Text style={Styles.dayofMonth}>{tomorrow3.getDate()}</Text> */}
            {/* <Icon  name ="chevron-right" size={22}  style={Styles.btn_navigation} /> */}
             ...
             
                {
                  route.params?.ID_goback===item.id ?
                    ...
                    <Animatable.View  animation="flash" duration={1000}> 
                          <Icon name="soccer-ball-o" style={[Styles.Icon_Football,{color:'purple'}]} />
                    </Animatable.View>
                
                  :getHour===item.TimeStart ? <Icon name="soccer-ball-o" style={[Styles.Icon_Football,{color:'green'}]} />
                  
                  : getHour<item.TimeStart ? 
                  <TouchableOpacity onPress={()=>{navigation.navigate("DetailsFootball",{id:item.id})}}>
                  <Icon name="plus" style={[Styles.Icon_Football,{color:'gray'}]} />
                  </TouchableOpacity>
                    : <Icon name="close"  style={[Styles.Icon_Football,{color:'red'}]} />
